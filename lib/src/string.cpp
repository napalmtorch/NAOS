...
void* memcpy_sse(uint8_t* dest, uint8_t* src, size_t n)
{
...
void* memset_sse(uint8_t* dest, uint32_t value, size_t n)
{
...
void* memset(void* dest, int c, size_t n)
{
    ...
    uint32_t* dest32 = (uint32_t*)dest;
    uint8_t*  dest8 = ((uint8_t*)dest) + num_dwords * 4;
    asm volatile("cld;rep stosl" : "+D"(dest32), "+c"(num_dwords) : "a"(c) : "memory");
    asm volatile("cld;rep stosb" : "+D"(dest8),  "+c"(num_bytes)  : "a"(c)     : "memory");
    return dest;
}
